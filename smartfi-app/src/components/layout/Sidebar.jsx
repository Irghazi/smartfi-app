...
export function Sidebar({ isOpen, setIsOpen }) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed top-0 left-0 h-screen w-64 bg-white border-r-4 border-black z-50 flex flex-col transition-transform duration-300 ease-in-out
        ${isOpen ? 'translate-x-0' : '-translate-x-full'}
      `}>
        {/* Branding (Atas) */}
        <div className="p-6 flex items-center gap-4 bg-white border-b-4 border-black shadow-[0px_4px_0px_0px_rgba(0,0,0,1)]">
          <div className="w-14 h-14 bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] flex items-center justify-center relative overflow-hidden">
            <Image src="/images/icon-192x192.png" alt="SmartFi Logo" fill sizes="56px" className="object-contain p-1" />
          </div>
          <div className="flex flex-col">
            <h2 className="font-bold text-xl text-black leading-tight tracking-tight uppercase">SmartFi</h2>
            <p className="text-[10px] uppercase tracking-wider text-black font-bold">Fintech AI</p>
          </div>
        </div>

        {/* Menu Navigasi */}
        <nav className="flex-1 px-4 py-6 flex flex-col gap-3 overflow-y-auto bg-white">
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`
                  ...
                  ${isActive 
                    ? 'bg-blue-300 text-black font-bold border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] translate-x-1' 
                    : 'bg-white text-black font-bold border-transparent hover:border-black hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:-translate-y-0.5'
                  }
                `}
                onClick={() => setIsOpen(false)}
              >
        ...
        <div className="p-4 border-t-4 border-black flex flex-col gap-3 bg-white">
          <Link
          ...
          <button 
            ...
            className="flex items-center gap-3 px-4 py-3 bg-red-400 text-black font-bold border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] active:translate-y-1 active:translate-x-1 active:shadow-none w-full text-left"
          >
            <span className="text-black">
